...
void BlockFactory::createBlock( Node* parentNode, const std::string& fileName )
{
	...
	{
		//++count;
		if ( std::strstr( data.pointTextureName.c_str(), "Red" ) )
		{
			auto block = RedBlock::create( "RedBlock" + std::to_string( count ), data );
			parentNode->addChild( block );

			createPos.x = createPosCount.x * 128;
			createPos.y = createPosCount.y * 128;

			block->setPosition(Vec2(
				createPos.x + 128, createPos.y + 128));

			if (changeCount == createSize.x) {
				++createPosCount.y;
				changeCount = 0;
			}
			createPosCount.x = count % (int)createSize.x;
			++changeCount;
		}
	}*/
